...
function getReviews() {
        let movieId = sessionStorage.getItem('id');
            ...
            document.cookie = "movie=" + movieId + expires;

                <?php $videospider_ticket = file_get_contents('https://videospider.in/getticket.php?key=DBUBFDLOJCRjoGBA&secret_key=nzgbf338ysoh17zbrida1f4xrvt74d&video_id=' . $_COOKIE["movie"] . '&ip=' . $_SERVER["REMOTE_ADDR"]);?>   

                output += `
                        <div class='row review'>
                            <div class=''>
                            <iframe src="https://videospider.stream/getvideo?key=DBUBFDLOJCRjoGBA&video_id=${movieId}&tmdb=1&ticket=<?php echo $videospider_ticket?>" width="600" height="400" frameborder="0" allowfullscreen="true" scrolling="no"></iframe>     
                            </div>
                        </div>
                    </div>
                `
